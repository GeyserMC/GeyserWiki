...
2. Extract the downloaded ZIP file, and put the extracted JAR file into the geyser_extensions folder in you're GeyserMC standalone installation.
3. Start Geyser standalone as you do with a normal Geyser install. EG: `java -Xms1024M -jar Geyser.jar` (More info on [Creating a Startup Script](/geyser/creating-a-startup-script/))
4. Shutdown the standalone Geyser instance, and make you're desired changes to the GeyserConnect configuration in `GeyserConnect` in the `geyser_extensions` folder.
5. Connect to it to make sure its all working.
